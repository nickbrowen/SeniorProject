...
```{r}
tweets_subset <- read.csv("tweets_subset.csv")
```
...
tweets %>% #filter(account_category %in% c("RightTroll", "LeftTroll", "NewsFeed", "HashtagGamer")) %>%
     ...
     facet_wrap(~account_category, ncol = 3, scales = "free") + labs(x = "Word")
     ggtitle("Top Words in Each Account Category")
```
Looking at the top words for Commercial, I see the words mainly have to do with some weight loss program, and also "stocks" are in the top 10. Also, NonEnglish doesn't seem useful. I'll consider focusing on the categories RightTroll, LeftTroll, NewsFeed, Fearmonger, HashtagGamer, and possibly Unknown
...
tweets %>% filter(!account_category %in% c("Commercial", "NonEnglish")) %>%
           count(word, sort = T) %>% top_n(20) %>% 
           mutate(word = reorder(word, n)) %>%
           ggplot(aes(word,n)) + geom_col() + coord_flip() +
           ggtitle("Top Words in RightTroll, Newsfeed, HashtagGamer, Fearmonger, Unknown")
```
