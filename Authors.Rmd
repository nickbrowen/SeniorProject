...
tweets %>% distinct(author, .keep_all = T) %>%
    ...
    ggplot(aes(account_category, prop_accountcat, fill = dominant)) + geom_col() +
      coord_flip() + 
      labs(x = "Proportion of Authors Dominance Classification in Each Account Category")
```
...
```{r}
tweet_updates %>% 
  filter(account_category %in% c("RightTroll", "LeftTroll", "NewsFeed", "HashtagGamer")) %>%    ggplot(aes(new_updates, fill = account_category)) + geom_histogram(bins = 60) +   
